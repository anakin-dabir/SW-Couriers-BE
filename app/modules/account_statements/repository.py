...
class AccountStatementRepository(BaseRepository):
    ...
    async def list_for_org(
        ...
        *,
        ...
        stmt = select(AccountStatement).where(AccountStatement.organization_id == organization_id)
    ...
    async def get_active_by_signature(
        ...
        stmt = select(AccountStatement).where(
            AccountStatement.organization_id == organization_id,
            AccountStatement.content_signature == content_signature,
...
class AccountStatementScheduleRepository(BaseRepository):
    ...
    async def list_for_org(self, organization_id: str) -> list[AccountStatementSchedule]:
        stmt = (
            ...
            .where(AccountStatementSchedule.organization_id == organization_id)
            .order_by(AccountStatementSchedule.created_at.desc())
        )
